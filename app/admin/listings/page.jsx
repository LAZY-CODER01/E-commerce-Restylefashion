...
function StatusBadge({ status }) {
  ...
  const display = normalized === "pending" ? "Pending" : normalized === "active" ? "Active" : "Rejected";
  return (
...
// ── Access Denied with one-click promote ──────────────
    ...
    <div className="flex-1 flex flex-col items-center justify-center py-24 gap-6 text-center px-4">
      ...
      <div>
        ...
        <p className="text-[14px] text-gray-500 max-w-sm">
          You need Admin access to manage listings. Click below to promote your account instantly.
        </p>
      </div>
      <button
        onClick={onPromote}
...
export default function AdminListingsPage() {
  ...
  const handleMakeAdmin = async () => {
    ...
    try {
      ...
      if (data.token) localStorage.setItem("restyle_token", data.token);
  ...
  return (
    <div className="h-[calc(100vh-80px)] bg-brand-light flex font-roboto w-full overflow-hidden">
      ...
      <main className="flex-1 flex flex-col px-4 md:px-9 py-6 md:py-8 overflow-y-auto h-[calc(100vh-80px)] w-full">
        ...
        {forbidden && <AccessDenied onPromote={handleMakeAdmin} promoting={promoting} />}
              ...
              {STATUS_TABS.map((tab) => (
                <button key={tab.value} onClick={() => setActiveTab(tab.value)}
                  className={`px-6 py-2.5 rounded-full text-[13px] font-bold border transition-all ${
                    activeTab === tab.value ? tab.color : "bg-white text-gray-400 border-gray-200 hover:border-gray-300"
                  }`}>
                  {tab.label}
                </button>
              ))}
              ...
              <div className={`flex flex-col gap-4 overflow-y-auto ${selectedProduct ? "w-full lg:w-[420px] shrink-0" : "w-full"}`}>
                {loading ? (
                  ...
                  products.map((product) => (
                    <div key={product._id}
                      className={`bg-white rounded-[24px] border p-5 flex flex-col gap-3 transition-all duration-200 ${
                        selectedProduct?._id === product._id
                          ? "border-brand-pink shadow-md shadow-brand-pink/10"
                          : "border-gray-100 hover:border-gray-200 hover:shadow-sm"
                      }`}>

                      {/* Card top — clickable for detail panel */}
                      <div className="flex gap-4 cursor-pointer" onClick={() => setSelectedProduct(product._id === selectedProduct?._id ? null : product)}>
                        {/* Thumbnail */}
                        <div className="w-[72px] h-[72px] rounded-[14px] overflow-hidden shrink-0 bg-gray-50 border border-gray-100 relative">
                          {product.imageUrl ? (
                        ...
                        <div className="flex flex-col flex-1 min-w-0 gap-1">
                          <div className="flex items-start justify-between gap-2">
                            <h3 className="text-[14px] font-bold text-brand-dark leading-snug line-clamp-1">{product.title}</h3>
                          ...
                          <p className="text-[12px] text-gray-400">
                            by <span className="font-bold text-brand-dark">{product.seller?.fullName || "Unknown"}</span>
                          </p>
                      ...
                      {product.status === "pending" && (
                        <div className="flex gap-2 pt-1 border-t border-gray-50">
                          <button
                            ...
                            className="flex-1 h-[38px] rounded-full border border-red-200 text-red-500 font-bold text-[12px] hover:bg-red-50 transition-all flex items-center justify-center gap-1.5 disabled:opacity-50"
                          >
                            <CancelOutlinedIcon sx={{ fontSize: 16 }} />
                            {actionLoading[product._id] === "rejected" ? "Rejecting…" : "Reject"}
                          </button>
                          <button
                            onClick={() => handleAction(product._id, "active")}
                            disabled={!!actionLoading[product._id]}
                            className="flex-1 h-[38px] rounded-full bg-brand-pink text-white font-bold text-[12px] hover:bg-brand-pink/90 transition-all shadow-sm shadow-brand-pink/20 flex items-center justify-center gap-1.5 disabled:opacity-50"
                          >
                            <CheckCircleOutlineIcon sx={{ fontSize: 16 }} />
                            {actionLoading[product._id] === "active" ? "Approving…" : "Approve & Go Live"}
                          </button>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <p className="text-[13px] text-gray-400 font-medium mt-1">
                          {selectedProduct.brand} · {selectedProduct.category} · {selectedProduct.condition}
                        </p>
                      </div>
              ...
              {selectedProduct && selectedProduct.status === "pending" && (
                <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-4 flex gap-4 z-50 shadow-xl">
                  <button
                    ...
                    <CancelOutlinedIcon sx={{ fontSize: 20 }} /> Reject
                  </button>
                  <button
                    onClick={() => handleAction(selectedProduct._id, "active")}
                    disabled={!!actionLoading[selectedProduct._id]}
                    ...
                    <CheckCircleOutlineIcon sx={{ fontSize: 20 }} /> Approve
                  </button>
                </div>
              )}
            </div>
