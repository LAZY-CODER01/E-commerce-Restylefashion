...
export async function completeDraftListingFlow(setUser) {
  ...
  if (!isIndividual && !sellerProfile?.businessName?.trim()) {
    return {
      ok: false,
      reason: "no_draft",
      message: "Your listing draft is incomplete. Add your business name, then try again.",
    };
  }
  ...
  try {
    const { data: profileData } = await api.put("/auth/seller-profile", {
      sellerType,
      businessName: isIndividual ? "" : String(sellerProfile.businessName || "").trim(),
    ...
    fd.append("brand", product.brand || "Unknown");
    fd.append("category", product.category);
    fd.append("condition", product.condition);
    fd.append("description", product.description);
    fd.append("price", product.sellingPrice);
    fd.append("originalPrice", product.retailPrice);
    ...
    if (!isIndividual && sellerProfile?.businessName?.trim()) {
      fd.append("businessName", sellerProfile.businessName.trim());
    }
